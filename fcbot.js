...
app.listen(process.env.PORT || 5000, function() {
  console.log("FoodCompositionBot: Webhook server is listening, port 5000");
});

// Server index page
app.get("/", (req, res) => {
  res.send(
    "FoodCompositionBot<br>More details: <a href='https://github.com/IuriiD/food_composition_chatbot_JS'>Github</a><br><a href='http://iuriid.github.io'>Iurii Dziuban - June 2018</a>"
  );
});
...
app.get("/webhook", function(req, res) {
  if (
    ...
    (process.env.fbVerifyToken || keys.fbVerifyToken)
  ) {
    console.log("FoodCompositionBot: Webhook verified");
    res.status(200).send(req.query["hub.challenge"]);
  } else {
    console.error(
      "FoodCompositionBot: Verification failed. Tokens do not match."
    );
    res.sendStatus(403);
  }
});
  ...
  if (userInput.type == "image") {
    ...
    try {
      ...
      if (userInput.payload == "GETTING_STARTED") {
        await send_text_message(
          senderId,
          "Hi! I'm a FoodCompositionBot. I can analyse food's composition by image"
        );
  ...
  try {
    if (caloriesData.status == "ok") {
      ...
      summary = `${food.charAt(0).toUpperCase() +
        food.slice(
          1
        )} contains approximately ${enercKCal100g} kcal (${enercKj100g} kj) per 100 grams or ${
        caloriesData.data.enercKCal
      } kcal (${caloriesData.data.enercKj} kj) per a standard serving (${
...
function vitaminNumbersToText(vitaminData) {
  ...
  try {
    if (vitaminData.status == "ok") {
      ...
      // Vitamin B1
      vitaminData.data.vitB1_404
        ? (vitB1 = vitaminData.data.vitB1_404)
        : (vitB1 = 0);

      // Vitamin B2
      vitaminData.data.vitB2_405
        ? (vitB2 = vitaminData.data.vitB2_405)
        : (vitB2 = 0);

      // Vitamin B5
      vitaminData.data.vitB5_410
        ? (vitB5 = vitaminData.data.vitB5_410)
        : (vitB5 = 0);

      // Vitamin B6
      vitaminData.data.vitB6_415
        ? (vitB6 = vitaminData.data.vitB6_415)
        : (vitB6 = 0);
      ...
      // Vitamin C
      vitaminData.data.vitC_401
        ? (vitC = vitaminData.data.vitC_401)
        : (vitC = 0);

      // Vitamin D
      vitaminData.data.vitD_324
        ? (vitDME = vitaminData.data.vitD_324)
        : (vitDME = 0);
      ...
      // Vitamin K
      vitaminData.data.vitK_430
        ? (vitK = vitaminData.data.vitK_430)
        : (vitK = 0);

      if (vitAmcg != 0) {
        vitAmcgIn100g =
          Math.floor(
            ((vitAmcg * 100) / vitaminData.data.servingWeightGrams) * 100
          ) / 100;
      } else {
        vitAmcgIn100g = 0;
      }
      ...
      if (vitAME != 0) {
        vitAMEIn100g =
          Math.floor(
            ((vitAME * 100) / vitaminData.data.servingWeightGrams) * 100
          ) / 100;
      } else {
        vitAMEIn100g = 0;
      }
      ...
      if (vitB1 != 0) {
        vitB1In100g =
          Math.floor(
            ((vitB1 * 100) / vitaminData.data.servingWeightGrams) * 100
          ) / 100;
      } else {
        vitB1In100g = 0;
      }
      ...
      if (vitB2 != 0) {
        vitB2In100g =
          Math.floor(
            ((vitB2 * 100) / vitaminData.data.servingWeightGrams) * 100
          ) / 100;
      } else {
        vitB2In100g = 0;
      }
      ...
      if (vitB5 != 0) {
        vitB5In100g =
          Math.floor(
            ((vitB5 * 100) / vitaminData.data.servingWeightGrams) * 100
          ) / 100;
      } else {
        vitB5In100g = 0;
      }
      ...
      if (vitB6 != 0) {
        vitB6In100g =
          Math.floor(
            ((vitB6 * 100) / vitaminData.data.servingWeightGrams) * 100
          ) / 100;
      } else {
        vitB6In100g = 0;
      }
      ...
      if (vitB12 != 0) {
        vitB12In100g =
          Math.floor(
            ((vitB12 * 100) / vitaminData.data.servingWeightGrams) * 100
          ) / 100;
      } else {
        vitB12In100g = 0;
      }
      ...
      if (vitC != 0) {
        vitCIn100g =
          Math.floor(
            ((vitC * 100) / vitaminData.data.servingWeightGrams) * 100
          ) / 100;
      } else {
        vitCIn100g = 0;
      }
      ...
      if (vitDmcg != 0) {
        vitDmcgIn100g =
          Math.floor(
            ((vitDmcg * 100) / vitaminData.data.servingWeightGrams) * 100
          ) / 100;
      } else {
        vitDmcgIn100g = 0;
      }
      ...
      if (vitDME != 0) {
        vitDMEIn100g =
          Math.floor(
            ((vitDME * 100) / vitaminData.data.servingWeightGrams) * 100
          ) / 100;
      } else {
        vitDMEIn100g = 0;
      }
      ...
      if (vitE != 0) {
        vitEIn100g =
          Math.floor(
            ((vitE * 100) / vitaminData.data.servingWeightGrams) * 100
          ) / 100;
      } else {
        vitEIn100g = 0;
      }
      ...
      if (vitK != 0) {
        vitKIn100g =
          Math.floor(
            ((vitK * 100) / vitaminData.data.servingWeightGrams) * 100
          ) / 100;
      } else {
        vitKIn100g = 0;
      }
      ...
      if (
        ...
        summary = `${food.charAt(0).toUpperCase() +
          food.slice(
            1
          )} doesn't contain vitamins or I failed to find information about them`;
      } else {
        summary = `${food.charAt(0).toUpperCase() +
          food.slice(1)} contains approximately (per 100 g):`;
      ...
      throw new Error(
        "Sorry but I failed to find vitamin data for the food you requested"
      );
    }
  } catch (error) {
...
async function totalSummary(food) {
  ...
  try {
    ...
    throw new Error(
      "Sorry but I failed to find data for the food you requested"
    );
  }
}
  ...
  return new Promise((resolve, reject) => {
    requestNullEncoding.get(imgUrl, function(error, response, body) {
      if (!error && response.statusCode == 200) {
        ...
        reject(
          "Sorry but I failed to recognize anything on the image provided"
        );
      }
    });
  });
    ...
    request(
      ...
      (error, response, body) => {
        if (
          ...
          !response.body.responses[0].error
        ) {
          body.responses[0].labelAnnotations.forEach(label =>
          ...
          console.log(
            `\nERROR from function googleVisionUrl():\n${error ||
              JSON.stringify(response.body.responses[0].error)}`
          );
          reject(
            "Sorry but I failed to recognize anything on the image provided"
          );
                ...
                elements: [
                  {
                    ...
                    image_url:
                      "https://s3.amazonaws.com/foodcompositionbot/fcb_logo.png",
                    buttons: [
                      {
                        type: "element_share",
                        share_contents: {
                          attachment: {
                            ...
                            payload: {
                              ...
                              elements: [
                                {
                                  title:
                                    "FoodCompositionBot - food analysis by image",
                                  image_url:
                                    "https://s3.amazonaws.com/foodcompositionbot/fcb_logo.png",
                                  default_action: {
                                    type: "web_url",
                                    url:
                                      "https://www.messenger.com/t/230099550919706"
                                  },
                                  buttons: [
                                    {
                                      type: "web_url",
